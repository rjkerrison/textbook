...
function calculateBallDeviation(paddle) {
  // ballDeviation - ball's distance from the centre of the paddle
  //   ballDeviation is always between -0.5 and 0.5
  const paddleMidpoint = (paddle.rightSide() + paddle.leftSide()) / 2
  const ballDeviation =
    (ball.position.x - paddleMidpoint) /
    (paddle.rightSide() - paddle.leftSide())
  return ballDeviation
}

function calculateDeflectionAngle(incomingAngle, ballDeviation) {
  // incomingAngle - current angle for the ball
  // ballDeviation - see calculateBallDeviation()
  const deflectionFactor = ballDeviation
  const incomingSign = Math.sin(incomingAngle) > 0 ? 1 : -1
  const outgoingSign = -incomingSign
  const incomingCos = Math.sin(incomingAngle)
  const outgoingCos = -incomingCos

  const newAngle =
    outgoingSign * (0.5 * Math.PI * deflectionFactor + Math.asin(outgoingCos))
  console.log({ deflectionFactor, ballDeviation, incomingAngle, newAngle })
  return newAngle
}

function detectCollisions() {
  if (Math.sin(ball.angle) > 0 && ball.position.y > playerPaddle.position.y) {
    if (playerPaddle.isOnPaddle(ball.position.x)) {
      // bounces near the edge should result in an angle deflected towards the horizontal
      const ballDeviation = calculateBallDeviation(playerPaddle)
      ball.angle = calculateDeflectionAngle(ball.angle, ballDeviation)
    } else {
      score.computer++
  ...
  if (Math.sin(ball.angle) < 0 && ball.position.y < computerPaddle.position.y) {
    if (computerPaddle.isOnPaddle(ball.position.x)) {
      const ballDeviation = calculateBallDeviation(computerPaddle)
      ball.angle = calculateDeflectionAngle(ball.angle, ballDeviation)
    } else {
      score.player++
      setBallInitialPosition()
